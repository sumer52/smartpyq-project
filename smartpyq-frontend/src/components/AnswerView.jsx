...
const renderInline = (text) => {
  ...
  return parts.map((p, i) => {
    ...
    if (p.startsWith('`') && p.endsWith('`') && p.length > 2) {
      return <code key={i} className="px-1 py-0.5 rounded bg-muted-100 text-accent text-[0.85em] font-mono">{p.slice(1, -1)}</code>;
    }
    if (p.startsWith('*') && p.endsWith('*') && p.length > 2) {
      return <em key={i} className="italic text-secondary">{p.slice(1, -1)}</em>;
    }
    return p;
  });
};
...
export const FormattedText = ({ text }) => {
  ...
  return (
    <div className="space-y-3 text-primary text-sm leading-relaxed">
      {blocks.map((b, i) => {
        if (b.type === 'ul') return (
          <ul key={i} className="list-disc pl-5 space-y-1.5 marker:text-accent">
            {b.items.map((it, j) => <li key={j}>{renderInline(it)}</li>)}
          </ul>
        );
        if (b.type === 'ol') return (
          <ol key={i} className="list-decimal pl-5 space-y-1.5 marker:text-accent">
            {b.items.map((it, j) => <li key={j}>{renderInline(it)}</li>)}
          </ol>
        );
        if (b.type === 'quote') return (
          <blockquote key={i} className="border-l-2 border-brand-300 pl-3 text-secondary italic">{renderInline(b.text)}</blockquote>
        );
        if (b.type === 'indent') return <p key={i} className="pl-6 text-secondary">{renderInline(b.text)}</p>;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent hideClose className="flex max-w-[95vw] flex-col gap-0 p-0 sm:max-w-[95vw]"
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#1C1B18]/50 rounded-t-2xl">
          <DialogTitle className="text-secondary text-sm font-normal truncate max-w-[50%]">{name || 'Answer image'}</DialogTitle>
          <div className="flex items-center gap-2">
            <button onClick={() => setZoom(z => Math.max(0.5, +(z - 0.25).toFixed(2)))}
              className="p-2 rounded-lg bg-muted-100 hover:bg-white/20 text-primary" aria-label="Zoom out"><MinusIcon className="h-4 w-4" /></button>
            <span className="text-secondary text-xs w-12 text-center tabular-nums">{Math.round(zoom * 100)}%</span>
            <button onClick={() => setZoom(z => Math.min(4, +(z + 0.25).toFixed(2)))}
              className="p-2 rounded-lg bg-muted-100 hover:bg-white/20 text-primary" aria-label="Zoom in"><PlusIcon className="h-4 w-4" /></button>
            <a href={src} target="_blank" rel="noreferrer"
              className="p-2 rounded-lg bg-muted-100 hover:bg-white/20 text-primary" aria-label="Open original"><ArrowTopRightOnSquareIcon className="h-4 w-4" /></a>
            <DialogClose
              className="p-2 rounded-lg bg-muted-100 hover:bg-white/20 text-primary focus:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-400/60"
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent hideClose className="flex max-w-[95vw] flex-col gap-0 p-0 sm:max-w-[95vw]"
        aria-describedby={undefined}>
        <div className="flex items-center justify-between px-4 py-3 bg-[#1C1B18]/50 rounded-t-2xl">
          <DialogTitle className="text-secondary text-sm font-normal truncate max-w-[50%]">{name || 'Answer document'}</DialogTitle>
          <div className="flex items-center gap-2">
            <a href={src} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted-100 hover:bg-white/20 text-primary text-xs">
...
const AnswerView = ({ answer, answerUrl }) => {
  ...
  if (!answer || !answer.answer_type) {
    return (
      <div className="bg-white border border-line rounded-xl p-4 text-muted text-sm italic">
  ...
  if (answer.answer_type === 'text') {
    return (
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
        <p className="text-success text-xs font-semibold mb-2 flex items-center gap-1.5">
  ...
  if (answer.answer_type === 'image') {
    ...
    return (
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
        <p className="text-success text-xs font-semibold mb-3 flex items-center gap-1.5">
        ...
        <button onClick={() => setLightboxOpen(true)} className="group block w-full" aria-label="View answer image">
          <div className="relative rounded-lg overflow-hidden border border-line bg-white max-h-80">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-muted-50 transition-colors">
              <span className="opacity-0 group-hover:opacity-100 transition-opacity inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#1C1B18]/50 text-primary text-xs">
    ...
    return (
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
        ...
        <button onClick={() => setPdfOpen(true)}
          className="w-full flex items-center gap-3 p-4 rounded-xl border border-line bg-white hover:bg-muted-100 transition-colors text-left">
          <span className="p-3 rounded-lg bg-red-500/20 text-error"><DocumentIcon className="h-6 w-6" /></span>
          <span className="flex-1 min-w-0">
            <span className="block text-primary text-sm font-medium truncate">{answer.answer_file_name || 'Answer document'}</span>
